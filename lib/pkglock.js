...
function resolveEntity (cache, scope, pkgName, filePath) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      fetchPackageSync(cache, scope.dependencies[pkgName], scope.dependencies[pkgName].integrity, pkgName)
      pkg = getPkg(cache, spec, scope, pkgName)
    } else {
      throw e
    }
...
function fetchPackageSync (cache, pkg, integrity, pkgName) {
  cp.spawnSync(process.argv[0], [
    __filename,
    'ensure-pkg',
    cache,
...
if (require.main === module && process.argv[2] === 'ensure-pkg') {
  let [cache, integrity, pkg, pkgName] = process.argv.slice(3)
  pkg = JSON.parse(pkg)
  if (!config) { config = require('./config.js') }
  ...
  opts.log.warn('fs', 'restoring broken or missing file from package', `${pkgName}@${pkg.version}`)
