...
class Installer {
  ...
  async checkLock () {
    ...
    if (!pkg._shrinkwrap || !pkg._shrinkwrap.lockfileVersion) {
      throw new Error(`tink can only install packages with an existing package-lock.json or npm-shrinkwrap.json with lockfileVersion >= 1. Run an install with npm@5 or later to generate it, then try again.`)
    }
    ...
    if (result.status) {
      ...
      throw new Error(
        'tink can only install packages when your package.json and package-lock.json or ' +
