...
function overrideNode () {
  ...
  fs.readdir = function (p, callback) {
    ...
    if (resolved == null && !wasFalse) { return readdir(p, callback) }
    if (!resolved) { notFoundError(p, callback) }
    const pkgPath = resolved.resolvedPath.replace(/package\.json$/, '')
    const dirPath = path.resolve(p).replace(/([/\\])?$/, '/')
      .replace(pkgPath, '') || '.'
    const files = Object.keys(resolved.pkg.files).filter(p => {
      return path.dirname(p) === dirPath
    })
  ...
  fs.readdirSync = function (p) {
    ...
    if (resolved == null && !wasFalse) { return readdirSync(p) }
    if (!resolved) { notFoundError(p) }
    const pkgPath = resolved.resolvedPath.replace(/package\.json$/, '')
    const dirPath = path.resolve(p).replace(/([/\\])?$/, '/')
      .replace(pkgPath, '') || '.'
    const files = Object.keys(resolved.pkg.files).filter(p => {
