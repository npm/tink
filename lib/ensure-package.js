...
async function ensurePackage (name, dep, opts) {
  ...
  if (integrity) {
    const info = await cacache.get.info(opts.cache, `cruxged-package:${INDEX_VERSION}:${integrity}`)
    if (info) {
  ...
  let unpacker = new CacacheUnpacker({
    strip: 1,
    cache: opts.cache,
    warn: err => process.emit('warn', 'crux', err.message)
  })
  if (!integrity) {
  ...
  return new Promise((resolve, reject) => {
    ...
    unpacker.on('finish', async () => {
      try {
        const key = `cruxged-package:${INDEX_VERSION}:${integrity}`
