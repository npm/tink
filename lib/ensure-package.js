    ...
    try {
      ...
      if (this.isResolvePkg && entry.path.match(/^lib\/node-modules-paths\.js$/)) {
        data = Buffer.from(data.toString('utf8').replace(
          ": ['node_modules']",
          ": ['node_modules', 'node_modules/.package-map.json'] // NOTE: THIS LINE WAS PATCHED BY tink"
        ), 'utf8')
      }
      if (this.isEnhancedResolvePkg && entry.path.match(/^lib\/ResolverFactory\.js$/)) {
        data = Buffer.from(data.toString('utf8').replace(
          'let modules = options.modules || ["node_modules"]',
          'let modules = (options.modules || ["node_modules"]).concat("node_modules/.package-map.json") // NOTE: THIS LINE WAS PATCHED BY tink!!'
        ), 'utf8')
      }
      if (this.isEnhancedResolvePkg && entry.path === 'lib/ModulesInHierarchicDirectoriesPlugin.js') {
        data = Buffer.from(data.toString('utf8').replace(
          'if(!err && stat && stat.isDirectory())',
          'if(addr.match(/[/\\\\]+\\.package-map\\.json$/i) || (!err && stat && stat.isDirectory())) // NOTE: THIS LINE WAS PATCHED BY tink'
        ))
      }
...
async function ensurePackage (name, dep, opts) {
  ...
  let unpacker = new CacacheUnpacker({
    strip: 1,
    cache: opts.cache,
    warn: err => process.emit('warn', 'tink', err.message)
  })
  if (!integrity) {
  ...
  return new Promise((resolve, reject) => {
    ...
    unpacker.on('finish', async () => {
      try {
        const key = `tinked-package:${INDEX_VERSION}:${integrity}`
